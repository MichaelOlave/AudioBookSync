...
class ErrorLog(Base):
    ...
    asin = Column(String(10), ForeignKey("books.asin", ondelete="SET NULL"), nullable=True, index=True)
    sync_id = Column(UUID(as_uuid=True), ForeignKey("sync_history.sync_id", ondelete="SET NULL"), nullable=True)
    ...
    timestamp = Column(DateTime(timezone=True), server_default="CURRENT_TIMESTAMP", nullable=False, index=True)
