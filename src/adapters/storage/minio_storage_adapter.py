"""MinIO file storage adapter for hexagonal architecture.

This module implements the FileStoragePort interface using MinIO as the storage backend.

Following hexagonal architecture principles:
- Port: FileStoragePort (interface defining storage operations)
- Adapter: MinIOStorageAdapter (this module - implements port for MinIO)
"""
...
class MinIOStorageAdapter(FileStoragePort):
    """MinIO adapter implementing FileStoragePort.

    Provides file storage operations using MinIO as the backend with per-user
    ...
    def __init__(self, minio_client: Optional[MinIOClient] = None):
        """Initialize MinIOStorageAdapter with MinIOClient.
        ...
        logger.info("MinIOStorageAdapter initialized")

    def _generate_object_key(
        self, file_type: str, asin: Optional[str] = None, title: Optional[str] = None
    ) -> str:
        """Generate MinIO object key for file storage.
    ...
    def _get_user_bucket(self, user_id: str) -> str:
        """Get MinIO bucket name for user.
    ...
    def ensure_user_bucket(self, user_id: str) -> bool:
        """Ensure MinIO bucket exists for user, create if not.
    ...
    def file_exists(self, user_id: str, object_key: str) -> bool:
        ...
        Args:
            user_id: User ID for bucket routing
            object_key: MinIO object key to check
        ...
        logger.debug(f"Checking if file exists in MinIO: user_id={user_id}, object_key={object_key}")

        exists = self.minio_client.file_exists(bucket_name, object_key)

        if exists:
            logger.debug(f"File exists in MinIO: user_id={user_id}, object_key={object_key}")
        else:
            logger.debug(f"File does not exist in MinIO: user_id={user_id}, object_key={object_key}")
